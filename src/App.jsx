...
const Sidebar = ({ onLogout, userProfile, onProfileClick, isOwner }) => {
  ...
  return (
    <aside className="w-64 bg-surfaceLight dark:bg-surfaceDark border-r border-borderLight dark:border-borderDark flex flex-col hidden md:flex transition-colors duration-200">
      ...
      <div className="p-4 border-t border-borderLight dark:border-borderDark">
        <button onClick={onProfileClick} className="flex items-center gap-3 w-full text-left px-3 py-2 mb-2 rounded-md hover:bg-pageLight dark:hover:bg-pageDark transition-colors group">
      ...
      <BrowserRouter>
        {!user ? (
          ...
          <DataProvider workspaceUid={workspaceUid}>
            <div className="flex h-screen bg-pageLight dark:bg-pageDark font-sans transition-colors duration-200">
              <Sidebar onLogout={handleLogout} userProfile={userProfile} onProfileClick={handleOpenProfile} isOwner={isOwner} />
              <main className="flex-1 flex flex-col overflow-hidden">
                <header className="h-16 bg-surfaceLight dark:bg-surfaceDark border-b border-borderLight dark:border-borderDark flex items-center justify-between px-6 transition-colors duration-200">
                  <h1 className="text-base font-medium text-textLight dark:text-textDark">System Workspace</h1>
              ...
              <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                <div className="bg-surfaceLight dark:bg-surfaceDark border border-borderLight dark:border-borderDark rounded-md shadow-2xl w-full max-w-md p-6 animate-in fade-in">
                  <div className="text-center mb-6">
                    ...
                    <h3 className="text-lg font-bold text-textLight dark:text-textDark">Change Default Password</h3>
                    <p className="text-sm text-mutedLight dark:text-mutedDark mt-1">
                      You are logging in with a default password. For security, please set a new personal password before continuing.
                    </p>
                  </div>

                  {forcedPasswordError && (
                    <div className="mb-4 px-3 py-2 text-sm rounded bg-red-50 text-red-700 border border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-900/30">
                      {forcedPasswordError}
                    </div>
                  )}
                  ...
                  <form onSubmit={handleForcedPasswordSubmit} className="space-y-4">
                    ...
                    <div className="pt-2">
                      <button type="submit" disabled={isUpdatingForcedPassword} className="w-full px-4 py-2 bg-primary text-white text-sm font-medium rounded hover:bg-primaryHover transition-colors shadow-sm disabled:opacity-50">
            ...
            {isProfileModalOpen && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                 <div className="bg-surfaceLight dark:bg-surfaceDark border border-borderLight dark:border-borderDark rounded-md shadow-xl w-full max-w-md flex flex-col animate-in fade-in">
                  <div className="px-5 py-3.5 border-b border-borderLight dark:border-borderDark flex items-center justify-between">
                    ...
                    <button onClick={() => setIsProfileModalOpen(false)} className="text-mutedLight dark:text-mutedDark hover:text-textLight dark:hover:text-textDark">
                  ...
                  <div className="p-5">
                    {profileStatus.message && (
                      <div className={`mb-4 px-3 py-2 text-sm rounded ${profileStatus.type === 'success' ? 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400 border border-green-200 dark:border-green-900/30' : 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400 border border-red-200 dark:border-red-900/30'}`}>
                        {profileStatus.message}
                      </div>
                    )}
                    <form id="profileForm" onSubmit={handleProfileUpdate} className="space-y-4">
                      <div>
                        ...
                        <p className="text-[11px] text-mutedLight dark:text-mutedDark mt-1">Email cannot be changed.</p>
                      </div>
                      <div>
                        ...
                        <input 
                          ...
                          disabled={!isOwner}
                        />
                        {!isOwner && <p className="text-[11px] text-mutedLight dark:text-mutedDark mt-1">Only administrators can change display names.</p>}
                      </div>
                      <div>
                        ...
                        <input 
                          ...
                          disabled={!isOwner}
                        />
                        {!isOwner && <p className="text-[11px] text-mutedLight dark:text-mutedDark mt-1">Only administrators can change profile photos.</p>}
                      </div>
                      <div className="pt-2 border-t border-borderLight dark:border-borderDark mt-2">
                        <label className={labelClass}>New Password <span className="text-mutedLight dark:text-mutedDark font-normal text-xs">(Leave blank to keep current)</span></label>
                  ...
                  <div className="px-5 py-3 border-t border-borderLight dark:border-borderDark bg-pageLight/50 dark:bg-pageDark/50 flex justify-end gap-3 rounded-b-md">
                    ...
                    <button type="submit" form="profileForm" disabled={isSavingProfile} className="px-3 py-1.5 bg-primary text-white text-sm font-medium rounded hover:bg-primaryHover transition-colors shadow-sm disabled:opacity-50">{isSavingProfile ? 'Saving...' : 'Save Changes'}</button>
                  </div>
                </div>
              </div>
            )}
